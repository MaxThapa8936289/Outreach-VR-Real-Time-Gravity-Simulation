...
//		- To colour the particles based on their speeds (magnitude)
//
// Assignment Object: NBodySim.cs within "Main Camera"
// 
// Notes: 
...
Shader "NBodySim/SpriteParticleShader" 
{
	...
	SubShader 
	{
		...
		StructuredBuffer<float4> _Positions;		// 4 vector array of position and mass data
		StructuredBuffer<float4> _Velocities;		// 4 vector array of velocity and [u] data
		float _MaxSpeed;							// Velocity for the top of the colour scale 
		...
		float4 frag (g2f IN) : COLOR
		{
			...
			float ColourScale = dot(vel.xyz, vel.xyz) / _MaxSpeed;
			...
			*/
			...
			col.a = 1.0f;								// gamma i.e. transparency
